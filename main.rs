...
impl event::EventHandler<ggez::GameError> for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        for hungry in self.hungry {
            let dino_x = self.dino_x[hungry];
            let dino_y = self.dino_y[hungry];
            let food_
        }
...
fn main() -> GameResult {
    ...
    let cb = ggez::ContextBuilder::new("Dino Beast", "Mac")
        .window_setup(ggez::conf::WindowSetup::default().title("Dino Beast").vsync(false))
